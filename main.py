...
def parse_bot_commands(slack_events):
    ...
    for event in slack_events:
        if event["type"] == "message" and not "subtype" in event:
            ...
            parsing = nlu_engine.parse(message)

            print(json.dumps(parsing, indent=2))
            print(parsing["intent"]["probability"])

            if (parsing["intent"]["probability"] > 0.75):
                command = parsing["intent"]["intentName"]

            if user_id == starterbot_id:
                return command, event["channel"]
    return command, None
